...
import com.tripplanner.LocationService.dto.response.SearchRespondeDTO;
...
public class SearchController {
  ...
  public ResponseEntity<List<SearchPOIAndDestinationDTO>> searchPOIAndDestinationByKeyword(
    @PathVariable String keyword
  ) {
    //        try {
    List<SearchPOIAndDestinationDTO> results = searchService.suggestSearchPOIAndDestinationByKeyword(
      keyword
    );
    if (results.isEmpty()) {
      return new ResponseEntity<>(HttpStatus.NOT_FOUND);
    }
    return new ResponseEntity<List<SearchPOIAndDestinationDTO>>(
      results,
      HttpStatus.OK
    );
    //        } catch (Exception e) {
  ...
  public ResponseEntity<List<SearchPOIAndDestinationDTO>> searchDestinationByKeyword(
    @PathVariable String keyword
  ) {
    //        try {
    List<SearchPOIAndDestinationDTO> results = searchService.suggestSearchPOIAndDestinationByKeyword(
  ...
  public ResponseEntity<SearchRespondeDTO> searchByKeyword(
    @PathVariable String keyword,
    @PathVariable int page
  ) {
    List<SearchPOIAndDestinationDTO> results = searchService.searchPOIAndDestinationByKeyword(
      keyword
    );
    try {
      // List<SearchPOIAndDestinationDTO> results = searchService.searchPOIAndDestinationByKeyword(keyword);
      Page<SearchPOIAndDestinationDTO> paging = searchService.listToPage(
        results,
        page,
        10
      );
  ...
  public ResponseEntity<SearchRespondeDTO> filterByKeywordAndType(
    @PathVariable String keyword,
    @PathVariable int page,
    ...
    try {
      ...
      results =
        searchService.filterPOIAndDestinationByType(
  ...
  public ResponseEntity<ArrayList<SearchPOIAndDestinationDTO>> searchPOIByKeyword(
    @PathVariable String keyword
  ) {
    ArrayList<SearchPOIAndDestinationDTO> results = searchService.searchPOIByKeyword(
      keyword
    );
