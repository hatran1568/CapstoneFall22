...
public class DestinationController {
  ...
  public ResponseEntity<DesDetailsDTO> getDestinationById(
    @PathVariable int id
  ) {
    try {
  ...
  public ResponseEntity<ArrayList<POIBoxDTO>> get3POIinDestination(
    @PathVariable int id
  ) {
    try {
  ...
  public ResponseEntity<ArrayList<GalleryImages>> getDestinationImages(
    @PathVariable int id
  ) {
    try {
      ArrayList<GalleryImages> images = destinationRepo.getDestinationImagesURL(
        id
      );
      if (images.isEmpty()) {
        return new ResponseEntity<>(HttpStatus.NOT_FOUND);
  ...
  public ResponseEntity<ArrayList<DesListDTO>> getDesListAdmin(
    ...
    try {
      ...
      destinations =
        destinationRepo.getDesListAdmin(filter, nameKey, page * 30, 30);
      return new ResponseEntity<>(destinations, HttpStatus.OK);
    } catch (Exception e) {
      return new ResponseEntity<>(HttpStatus.INTERNAL_SERVER_ERROR);
    }
  ...
  public ResponseEntity<Integer> getDesListAdminCount(
    @PathVariable("nameKey") String nameKey
  ) {
    try {
      int count;
  ...
  public ResponseEntity<ArrayList<DesSelectDTO>> getDesSelectOfPOI(
  ...
  @PreAuthorize("hasAuthority('Admin')")
  @RequestMapping(
    value = "/destination/poi/update/{poiId}",
    consumes = "application/json",
    produces = { "*/*" },
    method = RequestMethod.POST
  )
  public ResponseEntity<?> updateDesOfPOI(
    @RequestBody ArrayList<POIDesRequestDTO> poides,
    @PathVariable("poiId") int poiId
  ) {
    try {
      if (poides.get(0).getValue() == 0) return new ResponseEntity<>(
        HttpStatus.OK
      );
  ...
  @Transactional(rollbackFor = { Exception.class, Throwable.class })
  @RequestMapping(
    value = "/destination/delete/{desId}",
    consumes = "application/json",
    method = RequestMethod.POST
  )
  ...
  public ResponseEntity<DesAddUpdateDTO> getDesUpdate(
  ...
  public ResponseEntity<ArrayList<POIImageUpdateDTO>> getDesImageUpdate(
    @PathVariable("desId") int desId
  ) {
    try {
      ArrayList<POIImageUpdateDTO> images = destinationRepo.getDesImagesUpdate(
        desId
      );
      if (images.isEmpty()) {
  ...
  public ResponseEntity<?> updateDes(@RequestBody UpdateDesDTO destination) {
    try {
      ...
      destinationRepo.updateDes(
        destination.getDesId(),
        destination.getName(),
        destination.getDescription(),
        date,
        destination.getBelongTo()
      );
      return new ResponseEntity<>(HttpStatus.OK);
    } catch (Exception e) {
      return new ResponseEntity<>(HttpStatus.INTERNAL_SERVER_ERROR);
  ...
  public ResponseEntity<?> addImage(
    @PathVariable int desId,
    @PathVariable String description,
    @RequestPart("File") MultipartFile file
  ) throws Exception {
    try {
      String webViewLink = driveManager.uploadFile(
        file,
        "tripplanner/img/destination"
      );
      if (description.equals("*")) destinationRepo.addImage(
        desId,
        null,
        webViewLink
      ); else destinationRepo.addImage(desId, description, webViewLink);
      return new ResponseEntity<>(HttpStatus.OK);
    } catch (Exception e) {
      return new ResponseEntity<>(HttpStatus.INTERNAL_SERVER_ERROR);
    }
  ...
  @Transactional(rollbackFor = { Exception.class, Throwable.class })
  @RequestMapping(
    value = "/destination/deleteImg/{imgId}",
    produces = { "*/*" },
    method = RequestMethod.POST
  )
