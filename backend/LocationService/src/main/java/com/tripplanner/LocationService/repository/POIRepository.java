...
public interface POIRepository extends JpaRepository<POI, Integer> {
  ...
  ArrayList<POIofDestinationDTO> getPOIOfDestination(
    int desId,
    int start,
    int count,
    int rating
  ...
  ArrayList<POIofDestinationDTO> getPOIOfDestinationFilter(
    int desId,
    int catId,
    int start,
    int count,
  ...
  @Query(
    "select d.distance from Distance d where d.startStation.activityId = :from and d.endStation.activityId =:to"
  )
  Optional<Double> getDistanceBetweenTwoPOIs(int from, int to);

  @Query(
    "SELECT p from POI p where p.name like  CONCAT('%',:keyword,'%') and p.isDeleted=false"
  )
  ArrayList<POI> findPOISByKeyword(String keyword);

  @Query(
    "SELECT COUNT(p.activityId) FROM POI p join Rating r on p.activityId=r.POI.activityId GROUP BY p.activityId HAVING p.activityId=:id"
  )
  Optional<Integer> getNumberOfRateByActivityId(int id);

  @Query(
    value = "select r.rate_id as rateId, r.`comment` as `comment` , r.date_created as created, r.date_modified as modified,r.is_deleted as deleted,r.rate as rate, r.poi_id as poiId, r.user_id as userId from rating r join user u on r.user_id = u.user_id where r.poi_id = ?1",
    nativeQuery = true
  )
  ...
  @Transactional
  @Query(
    value = "update rating r  set r.rate=?1,r.`comment`=?2,r.date_modified=?3 where r.user_id =?4 and r.poi_id=?5",
    nativeQuery = true
  )
  ...
  @Transactional
  @Query(
    value = "insert into rating (comment, date_created, date_modified, is_deleted, rate, poi_id, user_id)\n" +
    "values (:comment, :created, :modified, false, :rate, :poiId, :uid)",
    nativeQuery = true
  )
  ...
  @Query(
    value = "SELECT pi.url from poi p join poi_image pi on p.activity_id=pi.poi_id  where p.activity_id=:id limit 1",
    nativeQuery = true
  )
  Optional<String> getThumbnailById(int id);

  @Query(
    "SELECT p FROM POI p left join MasterActivity m on p.activityId = m.activityId where p.activityId = :masterActivityId"
  )
  ...
  Optional<ArrayList<POI>> getHotelByDestination(
    double distance,
    int src,
    double maxRate,
    double minRate,
  ...
  Optional<ArrayList<POI>> getHotelByPriceAndRate(
    double maxRate,
    double minRate,
    double maxPrice,
    double minPrice
  ...
  ArrayList<POIListDTO> getPOIListAdmin(
    String filter,
    int categoryId,
    String nameKeyword,
    int page,
  ...
  @Query(
    "Select p from POI p join POIDest pd on p.activityId = pd.poi.activityId where pd.destination.destinationId=:id and pd.poi.category.categoryID <>10 and pd.poi.category.categoryID <>11 and pd.poi.isDeleted=false"
  )
  public ArrayList<POI> getPOIsByDestinationId(int id);

  @Query(
    "Select p from POI p join POIDest pd on p.activityId = pd.poi.activityId where  pd.poi.category.categoryID <>10 and pd.poi.category.categoryID <>11 and pd.poi.isDeleted = false"
  )
  public ArrayList<POI> getPOIsByButHotel();

  @Query(
    "Select p from POI p join POIDest pd on p.activityId = pd.poi.activityId where pd.poi.category.categoryID <>11 and pd.poi.isDeleted = false"
  )
  public ArrayList<POI> getPOIs();

  @Query(
    "Select p from POI p join POIDest pd on p.activityId = pd.poi.activityId where  pd.poi.category.categoryID =10 and pd.poi.isDeleted = false"
  )
