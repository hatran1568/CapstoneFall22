...
import com.tripplanner.BlogService.dto.response.AdminCountTripDTO;
...
import com.tripplanner.BlogService.repository.AdminRepository;
...
public class AdminController {
  ...
  public ResponseEntity<AdminCountTripDTO> countTripByDays(
    ...
    try {
      ...
      DateTimeFormatter formatterSQL = DateTimeFormatter.ofPattern(
        "yyyy-MM-dd"
      );
  ...
  public ResponseEntity<ArrayList<AdminCountTripDTO>> countUserByDays(
    @PathVariable("start") String start,
    @PathVariable("end") String end
  ) {
    try {
      ...
      for (int i = 0; (startDate.compareTo(endDate.minusDays(i)) <= 0); i++) {
        ...
        labels.add(todayFormated);
        data.add(
          adminRepo.getUserCountByDay(todayFormatedSQL, tmrFormatedSQL, "LOCAL")
        );
      }
      AdminCountTripDTO count = new AdminCountTripDTO() {
        @Override
