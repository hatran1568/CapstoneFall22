...
public class BlogController {
  ...
  public ResponseEntity<BlogDetailsDTO> getBlogDetailsById(
  ...
  public ResponseEntity<ListBlogDTO> getBlogByKeyword(
  ...
  public ResponseEntity<ArrayList<BlogListDTO>> searchBlogsAdmin(
    @PathVariable String filter,
    @PathVariable int page
  ) {
    try {
      ArrayList<BlogListDTO> blogs = null;
      if (filter.equals("*")) blogs =
        blogRepo.getBlogsByKeywordAll("", page * 10, 10); else blogs =
        blogRepo.getBlogsByKeywordAll(filter, page * 10, 10);
      return new ResponseEntity<>(blogs, HttpStatus.OK);
    } catch (Exception e) {
      return new ResponseEntity<>(HttpStatus.INTERNAL_SERVER_ERROR);
    }
  ...
  public ResponseEntity<Integer> searchBlogsAdminCount(
    @PathVariable String filter,
    @PathVariable int page
  ) {
    try {
      ...
      if (filter.equals("*")) count =
        blogRepo.getBlogsByKeywordAllCount("", page * 10, 10); else count =
        blogRepo.getBlogsByKeywordAllCount(filter, page * 10, 10);
  ...
  public ResponseEntity<ArrayList<BlogNearbyDTO>> getNearbyBlogs(
    @PathVariable int blogId
  ) {
    try {
      ...
      if (blogId == firstBlog.getBlogId()) blogs.add(0, lastBlog);
      if (blogId == lastBlog.getBlogId()) blogs.add(firstBlog);
      return new ResponseEntity<>(blogs, HttpStatus.OK);
    } catch (Exception e) {
      return new ResponseEntity<>(HttpStatus.INTERNAL_SERVER_ERROR);
    }
  ...
  public ResponseEntity<?> updateBlog(@RequestBody BlogAddUpdateDTO blog) {
    try {
      ...
      String content = " ";
      String title = " ";
      if (blog.getThumbnail().length() > 0) thumbnail = blog.getThumbnail();
      if (blog.getContent().length() > 0) content = blog.getContent();
      if (blog.getTitle().length() > 0) title = blog.getTitle();
