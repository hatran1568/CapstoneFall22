...
public interface BlogRepository extends JpaRepository<Blog, Integer> {
  @Query(
    "SELECT b from Blog b where b.title like  CONCAT('%',:keyword,'%') AND b.status = 'PUBLISHED'"
  )
  ArrayList<Blog> getBlogsByKeyword(String keyword);
  ...
  ArrayList<BlogListDTO> getBlogsByKeywordAll(
    String keyword,
    int start,
    int count
  );
  ...
  @Query(
    ...
    " order by b.date_modified desc limit 3",
    nativeQuery = true
  )
