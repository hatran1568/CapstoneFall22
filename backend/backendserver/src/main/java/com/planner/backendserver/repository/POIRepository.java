...
public interface POIRepository extends JpaRepository<POI, Integer> {
  ...
  @Query(
    "select r from Rating r join User u on r.user.userID = u.userID where r.POI.activityId = :id"
  )
  ...
  @Transactional
  @Query(
    "update Rating r set r.rate = :rate, r.Comment = :comment, r.dateModified = :modified where r.user.userID = :uid and r.POI.activityId = :poiId"
  )
  ...
  @Query(
    "Select p from POI p join POIDest pd on p.activityId = pd.poi.activityId where pd.destination.destinationId=:id and pd.poi.category.categoryID <>10 and pd.poi.category.categoryID <>11"
  )
