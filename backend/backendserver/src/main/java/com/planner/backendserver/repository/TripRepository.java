...
public interface TripRepository extends JpaRepository<Trip, Integer> {
  ...
  @Query(
    "select t from Trip t where t.tripId = :id and ((t.status = 'PUBLIC') or (t.user.userID = :userId and t.status = 'PRIVATE'))"
  )
  ...
  @Query(
    "select t from Trip t where t.user.userID = :id and t.status <>'DELETED' order by t.dateModified desc"
  )
  ...
  @Query(
    "SELECT COUNT(t) FROM Trip t WHERE t.user.userID = :id and t.status <>'DELETED'"
  )
  ...
  @Query(
    value = "select * from trip t where t.status='PUBLIC' and t.name like CONCAT('%',:name,'%') order by t.date_created desc limit :limit offset :offset",
    nativeQuery = true
  )
  ...
  public int getPublicTripsCount(
    String name,
    int minDays,
    int maxDays,
    Date earliest
  ...
  public ArrayList<Trip> getPublicTripsWithSearch(
    int offset,
    int limit,
    String name,
    int minDays,
