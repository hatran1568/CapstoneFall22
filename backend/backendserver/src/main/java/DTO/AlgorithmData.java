...
public class AlgorithmData {
    ...
    public AlgorithmData(Date _startDate, Date _endDate, double _distanceOfPOI[][], POI[] _POIs,int _numberOfPOI,int[] _userPreference,int _budget,int _start,int _end){
        ...
        this.dayOfTrip  = (int) TimeUnit.DAYS.convert(diff/(24*3600*1000), TimeUnit.DAYS);
    ...
    public void recalculateRate() {
        for (int i = 0; i < numberOfPOI; i++) {
            int finalI = i;
            if(Arrays.stream(userPreference).anyMatch(n->n==POIs[finalI].getCategory().getCategoryID())){
