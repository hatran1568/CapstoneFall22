...
public class TripController {
  ...
  @GetMapping("/{id}")
  public ResponseEntity<DetailedTripDTO> getTripById(
    @PathVariable int id,
  ...
  public ResponseEntity<TripGeneralDTO> getTripGeneralById(
    @PathVariable int id
  ) {
    try {
  ...
  public ResponseEntity<TripGeneralDTO> cloneTrip(
  ...
  public ResponseEntity<?> updateStartAndEndDates(
  ...
  public ResponseEntity<Double> getDistanceBetweenTwoPOIs(
  ...
  public ResponseEntity<TripDetailDTO> addTripDetail(
    @RequestBody ObjectNode objectNode
  ) {
    //        try{
    ...
    TripDetailDTO result = tripService.addTripDetail(
      date,
      startTime,
      endTime,
      activityId,
  ...
  public ResponseEntity<Integer> addTripDetailGenerated(
    @RequestBody ObjectNode objectNode
  ) {
    //        try{
    ...
    int activityId = objectNode.get("activityId").asInt();
    String note = objectNode.get("note").asText();
    Integer result = tripService.addTripDetailGenerated(
      date,
      startTime,
      endTime,
      activityId,
  ...
  public ResponseEntity<TripDetailDTO> addCustomTripDetail(
    @RequestBody ObjectNode objectNode
  ) {
    //        try{
    Date date = Date.valueOf(objectNode.get("date").asText());
    ...
    String note = objectNode.get("note").asText();
    TripDetailDTO result = tripService.addCustomTripDetail(
      date,
      startTime,
      endTime,
      tripId,
  ...
  public ResponseEntity<List<TripDetailDTO>> getDetailsToDelete(
    @RequestBody ObjectNode objectNode
  ) {
    //        try{
    int tripId = objectNode.get("tripId").asInt();
    int numberOfDays = objectNode.get("numberOfDays").asInt();
    List<TripDetailDTO> tripDetailDTOS = tripService.getTripDetailsToBeDeleted(
      tripId,
      numberOfDays
    );
  ...
  public ResponseEntity<TripDetailDTO> editTripDetail(
    @RequestBody TripDetailDTO newDetail,
    @RequestParam int id
  ) {
    try {
  ...
  public ResponseEntity<TripDetailDTO> editCustomTripDetail(
    @RequestBody TripDetailDTO newDetail,
    @RequestParam int detailId,
    @RequestParam int tripId
  ) {
    //        try{
    TripDetailDTO detail = tripService.editCustomTripDetailById(
      newDetail,
      detailId,
      tripId
    );
  ...
  public ResponseEntity<TripDetails> deleteTripDetail(
    @RequestBody ObjectNode objectNode
  ) {
    //        try{
  ...
  public ResponseEntity<?> getTripsByUser(
    ...
    return new ResponseEntity<>(
      tripService.getTripsByUser(userId, page, 10),
      HttpStatus.OK
    );
    //        } catch (Exception e) {
    //            return new ResponseEntity<>(HttpStatus.INTERNAL_SERVER_ERROR);
  ...
  public ResponseEntity<?> createEmptyTrip(@RequestBody TripDTO tripDTO) {
    //        try{
    TripGeneralDTO result = tripService.createEmptyTrip(
      tripDTO.getBudget(),
      tripDTO.getName(),
      tripDTO.getUserId(),
  ...
  public ResponseEntity<?> getTripsForHomepage(@PathVariable int userId) {
    //        try{
    return new ResponseEntity<>(
      tripService.getLast3TripsByUser(userId),
      HttpStatus.OK
    );
    //        } catch(Exception e){
    //            return new ResponseEntity<>(HttpStatus.INTERNAL_SERVER_ERROR);
  ...
  public ResponseEntity<?> getTotalTrips(@PathVariable int userId) {
    try {
      return new ResponseEntity<>(
        tripService.countTripByUser(userId),
        HttpStatus.OK
      );
    } catch (Exception e) {
      return new ResponseEntity<>(HttpStatus.INTERNAL_SERVER_ERROR);
  ...
  @Transactional(rollbackFor = { Exception.class, Throwable.class })
  @PostMapping("/insert")
  public ResponseEntity<Integer> insertTrip(
    @RequestBody TripGenerateDTO input
  ) {
    tripRepo.createEmptyTrip(
      new Date(Calendar.getInstance().getTime().getTime()),
      new Date(Calendar.getInstance().getTime().getTime()),
      TripStatus.PRIVATE.name(),
      input.getBudget(),
      input.getName(),
      input.getUserId(),
      input.getStartDate(),
  ...
  public ResponseEntity<?> getPublicTripsCount(
    @RequestParam(required = false) String search,
    @RequestParam(required = false) Integer minDays,
    @RequestParam(required = false) Integer maxDays
  ) {
    try {
      ...
      cal.add(Calendar.MONTH, -1);
      Date earliest = new Date(cal.getTime().getTime());
      int count = tripService.countPublicTrips(
        search,
        minDays,
        maxDays,
        earliest
  ...
  public ResponseEntity<?> getTripsForHomepageGuest(@RequestBody int[] array) {
    //        try{
    return new ResponseEntity<>(
      tripService.getLast3TripsByGuest(array),
      HttpStatus.OK
    );
    //        } catch(Exception e){
    //            return new ResponseEntity<>(HttpStatus.INTERNAL_SERVER_ERROR);
