...
public interface ExpenseRepository
  extends JpaRepository<ExpenseCategory, Double> {
  @Query(
  ...
  @Transactional
  @Query(
    value = "insert into trip_expense (amount,description,expense_category_id,trip_id,trip_details_id) VALUE(?1,?2,8,?3,?4)",
    nativeQuery = true
  )
  void insertExpense(
    double amount,
  ...
  @Transactional
  @Query(
    value = "delete te from trip_expense te left join trip_details td on te.trip_details_id = td.trip_details_id where td.trip_id=:tripId and td.day_number > :numberOfDays",
    nativeQuery = true
  )
  ...
  @Transactional
  @Query(
    value = "delete te from trip_expense te where te.trip_details_id=:detailsId",
    nativeQuery = true
  )
