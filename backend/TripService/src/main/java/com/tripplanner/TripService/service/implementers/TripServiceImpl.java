...
public class TripServiceImpl implements TripService {
  ...
  public DetailedTripDTO getDetailedTripById(int tripId, int userId) {
    ...
    if (
      trip.getStatus() == TripStatus.PRIVATE && trip.getUser() != userId
    ) return null;
    List<TripDetailsQueryDTO> tripDetailedDTO = tripDetailRepository.getTripDetailsByTrip(
      tripId
    );
  ...
  private void cloneTripDetails(int tripId, int newTripId) {
    ...
    for (TripDetails detail : tripDetails) {
      ...
      if (Boolean.TRUE.equals(isExist)) {
        ...
        tripRepository.insertTripDetails(
          detail.getDayNumber(),
          detail.getEndTime(),
          detail.getNote(),
          detail.getStartTime(),
  ...
  public void addPOICostToExpenses(int tripId, int activityId, int detailId) {
    ...
    ServiceInstance instance = instances.get(0);
    com.tripplanner.TripService.dto.response.POIDTO poidto = restTemplateClient
      .restTemplate()
      .getForObject(
  ...
  public TripDetailDTO addCustomTripDetail(
    ...
    TripDetailDTO saved = addTripDetail(
      date,
      startTime,
      endTime,
      id,
  ...
  public TripListDTO getTripsByUser(int userId, int page, int size) {
    ...
    Page<TripGeneralDTO> paging = listToPage(
      trips
        .stream()
        .map(trip -> {
  ...
  private Page<TripGeneralDTO> listToPage(
  ...
  public List<TripDetailDTO> getTripDetailsToBeDeleted(
    ...
    for (TripDetails tripDetail : tripDetails) {
      TripDetailDTO tripDetailDTO = mapper.map(tripDetail, TripDetailDTO.class);
      MasterActivityDTO masterActivityDTO = getMasterActivity(
        tripDetail.getMasterActivity()
      );
  ...
  public ArrayList<PublicTripDTO> getPublicTrips(
    ...
    ArrayList<Trip> trips = tripRepository.getPublicTrips(
      page * pageSize,
      pageSize,
      search,
      minDays,
    ...
    for (Trip trip : trips) {
      ...
      dto.setDestinations(
        tripRepository.getDestinationsOfTrip(trip.getTripId())
      );
      dto.setNumberOfDays(
        getDayNumberByFromStartDate(
          (Date) trip.getStartDate(),
  ...
  @Override
  public TripGeneralDTO createEmptyTrip(
    Double budget,
    String name,
    int userId,
    Date startDate,
