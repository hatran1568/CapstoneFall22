...
public interface RequestRepository extends JpaRepository<OptimizerRequest, Long> {
    @Query(value = "select * from optimize_request o where o.request_id=?1", nativeQuery = true)
    ...
    @Modifying
    @Transactional
    @Query(value = "Update `user`  set optimizer_request_id= null where user_id=?1", nativeQuery = true)
    ...
    @Modifying
    @Transactional
    @Query(value = "update optimize_request set `status` = ?1 where request_id =?2", nativeQuery = true)
