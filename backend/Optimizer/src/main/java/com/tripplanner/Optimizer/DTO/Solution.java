...
public class Solution {
  ...
  public boolean isValid(ArrayList<ArrayList<Integer>> gene) {
    ...
    for (int i = 0; i < data.getNumberOfPOI(); i++) {
      ...
      for (int j = 0; j < trip.size(); j++) {
        ...
        if (j == 0) {
          A[j][i] =
            data.getDailyStartTime()[i] + data.getPOIs()[poi].getDuration();
        } else {
          A[j][i] = A[j - 1][i] + data.getPOIs()[poi].getDuration();
        }
        if (
          A[j][i] >= data.getPOIs()[poi].getCloseTime() ||
          A[j][i] -
          data.getPOIs()[poi].getDuration() <=
          data.getPOIs()[poi].getOpenTime()
        ) {
    ...
    for (int i = 0; i < data.getDayOfTrip(); i++) {
      if (this.gene.get(i).size() > 1) {
        for (int j = 0; j < this.gene.get(i).size() - 1; j++) {
          distance +=
            data.getDistanceOfPOI()[this.gene.get(i).get(j)][this.gene.get(i)
                .get(j + 1)];
        }
      }
    }
  ...
  public double cal_waiting_time_obj() {
    ...
    for (int i = 0; i < data.getDayOfTrip(); i++) {
      if (this.gene.get(i).size() > 0) {
        double current_time =
          data.getDailyStartTime()[i] +
          data.getPOIs()[this.gene.get(i).get(0)].getDuration();
        for (int j = 1; j < this.gene.get(i).size(); j++) {
          if (
            current_time +
            data.getDistanceOfPOI()[this.gene.get(i).get(j - 1)][this.gene.get(
                  i
                )
                .get(j)] *
            90 <
            data.getPOIs()[this.gene.get(i).get(j)].getOpenTime()
          ) {
            waiting_time +=
              data.getPOIs()[this.gene.get(i).get(j)].getOpenTime() -
              current_time +
              data.getDistanceOfPOI()[this.gene.get(i)
                  .get(j - 1)][this.gene.get(i).get(j)] *
              90;
          }
          current_time =
            Double.max(
            ...
            data.getPOIs()[this.gene.get(i).get(j)].getDuration();
        }
      }
    }
    return waiting_time;
  ...
  public double cal_fitness() {
    ...
    fitness +=
      Math.pow(
        (cal_distance_obj() - data.getMinDistance()) /
        (data.getMaxDistance() - data.getMinDistance()),
        2
      ) *
      data.getW1();
    fitness +=
      Math.pow(
        (data.getMinWaitingTime() - cal_waiting_time_obj()) /
        (data.getMaxWaitingTime() - data.getMinWaitingTime()),
        2
      ) *
      data.getW2();
    fitness +=
      Math.pow(
        (cal_hapiness_obj() - data.getMaxHappiness()) /
        (data.getMaxHappiness() - data.getMinHappiness()),
        2
      ) *
      data.getW3();
    fitness +=
      Math.pow(
        (cal_number_of_destination_obj() - data.getMaxNumberOfDestinations()) /
        (data.getMaxNumberOfDestinations() - data.getMinNumberOfDestinations()),
        2
      ) *
      data.getW4();
    ...
    for (ArrayList<Integer> list : gene) {
      ...
      for (int i = 0; i < list.size(); i++) {
        ...
        tripDetails.setStartTime(
          (int) Math.max(
            currentTime,
            ((POI) tripDetails.getMasterActivity()).getOpenTime()
          )
        );
        currentTime =
          (int) (
            ...
            ((POI) tripDetails.getMasterActivity()).getDuration()
          );
        tripDetails.setEndTime((int) currentTime);
        if (i != list.size() - 1) {
          currentTime +=
            data.getDistanceOfPOI()[list.get(i)][list.get(i + 1)] * 270;
        }
