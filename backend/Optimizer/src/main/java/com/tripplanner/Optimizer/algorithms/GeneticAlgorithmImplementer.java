package com.tripplanner.Optimizer.algorithms;

import com.tripplanner.Optimizer.DTO.Data;
...
public class GeneticAlgorithmImplementer {
    ...
    for (int i = 0; i < data.getDayOfTrip(); i++) {
      ...
      double time =
        Double.max(
          ...
          data.getPOIs()[fullTrip.get(0)].getOpenTime()
        ) +
        data.getPOIs()[fullTrip.get(0)].getDuration();
      ...
      if (cost <= data.getDailyBudget()[i]) {
        ...
        while (true) {
          if (
            Double.max(
              time + data.getDistanceOfPOI()[current][fullTrip.get(0)] * 90,
              data.getPOIs()[fullTrip.get(0)].getOpenTime()
            ) +
            data.getPOIs()[fullTrip.get(0)].getDuration() <
            data.getDailyEndTime()[i] &&
            cost +
            data.getPOIs()[fullTrip.get(0)].getTypicalPrice() <
            data.getDailyBudget()[i]
          ) {
            time =
              Double.max(
                time + data.getDistanceOfPOI()[current][fullTrip.get(0)] * 90,
        ...
        if (
          ...
          time =
            Double.max(
              time +
              Double.max(
                data.getDistanceOfPOI()[current][fullTrip.get(0)] * 150,
                600
              ),
              data.getPOIs()[fullTrip.get(0)].getOpenTime()
            ) +
  ...
  public Solution mutation(Solution s, Data data) {
    ...
    if (s.gene.get(tripNumber).size() > 0) {
      ...
      double time =
        Double.max(
          data.getDailyStartTime()[tripNumber],
          data.getPOIs()[s.gene.get(tripNumber).get(0)].getOpenTime()
        ) +
        data.getPOIs()[s.gene.get(tripNumber).get(0)].getDuration();
      double cost = data
        .getPOIs()[s.gene.get(tripNumber).get(0)].getTypicalPrice();
      for (int i = 0; i < cutoffPoint; i++) {
        ...
        time =
          Double.max(
            time +
            Double.max(
              data.getDistanceOfPOI()[currentPOI][s.gene
                  .get(tripNumber)
                  .get(i + 1)] *
              150,
              600
            ),
            data.getPOIs()[s.gene.get(tripNumber).get(i + 1)].getOpenTime()
          ) +
          data.getPOIs()[s.gene.get(tripNumber).get(i + 1)].getDuration();
      ...
      while (fullTrip.size() > 0) {
        ...
        if (
          ...
          data.getPOIs()[fullTrip.get(0)].getDuration() <
          data.getDailyEndTime()[tripNumber] &&
          cost +
          data.getPOIs()[fullTrip.get(0)].getTypicalPrice() <
          data.getDailyBudget()[tripNumber]
        ) {
          time =
            Double.max(
              time + data.getDistanceOfPOI()[current][fullTrip.get(0)] * 90,
        ...
        while (fullTrip.size() > 0) {
          ...
          if (
            !(
              Double.max(
                newTime +
                data.getDistanceOfPOI()[currentPOI][fullTrip.get(0)] *
                90,
                data.getPOIs()[fullTrip.get(0)].getOpenTime()
              ) +
              data.getPOIs()[fullTrip.get(0)].getDuration() >=
  ...
  public Solution implementGA(Data data) {
    ...
    for (int j = 1; j <= 200; j++) {
      ...
      for (int i = 0; i < 900; i++) {
        ...
        nextPopulation.set(
          choosen,
          mutation2(nextPopulation.get(choosen), data)
        );
      }
      Collections.sort(
        nextPopulation,
