...
public class GenerateTripIml implements GenerateTrip {
  ...
  public CompletableFuture<ComplexResponse> generateTrip(
    ...
    try {
      listPoi =
        restTemplateClient
          ...
          .getForObject(
            instance.getUri() +
            "/location/api/pois/poisByDestination/" +
      ...
      List<ServiceInstance> userInstances = discoveryClient.getInstances(
        "user-service"
      );
      ...
      ResponseEntity<UserDetailResponseDTO> user = restTemplateClient
        .restTemplate()
        .exchange(
          userInstance.getUri() +
          "/user/api/user/findById/" +
          input.getUserId(),
          HttpMethod.GET,
          request2,
      ...
      String emailContent =
        ...
        "\n" +
        "Xin hãy thử lại sau";
      mailSender.sendSimpleMessageError(
        user.getBody().getEmail(),
        "Chuyến đi của bạn đã bị lỗi.",
        emailContent
      );
      return null;
    }
    ...
    for (int i = 0; i < numberOfPOI; i++) {
      for (int j = 0; j < numberOfPOI; j++) {
        if (i == j) distanceOfPOI[i][j] = 0; else {
          distanceOfPOI[i][j] =
            restTemplateClient
              .restTemplate()
              .getForObject(
                ...
                POIs[i].getActivityId() +
                "/" +
                POIs[j].getActivityId(),
                Double.class
              );
        }
      }
  ...
  public CompletableFuture<ComplexResponse> insertToDB(
    ...
    CompletableFuture<ComplexResponse> task = new CompletableFuture<>();
    List<ServiceInstance> locationInstances = discoveryClient.getInstances(
      "location-service"
    );
    HttpHeaders headers = new HttpHeaders();
    List<ServiceInstance> instances = discoveryClient.getInstances(
    ...
    DesDetailsDTO destination = restTemplateClient
      .restTemplate()
      .getForObject(
        locationInstance.getUri() +
    ...
    int id = restTemplateClient
      .restTemplate()
      .postForObject(
        instance.getUri() + "/trip/insert",
        request,
        Integer.class
      );
    ...
    for (TripDetails poi : tour.getListTripDetails()) {
      ...
      detailsGenerateDTO.setActivityId(
        String.valueOf(poi.getMasterActivity().getActivityId())
      );
      detailsGenerateDTO.setNote("");
      String nodeObject = gson.toJson(detailsGenerateDTO);
      HttpEntity<String> request1 = new HttpEntity<String>(nodeObject, headers);
      int poiId = restTemplateClient
        .restTemplate()
        .postForObject(
          instance.getUri() + "/trip/add-detail-generated",
          request1,
          Integer.class
        );
      headers = new HttpHeaders();
      ...
      restTemplateClient
        .restTemplate()
        .postForObject(
          instance.getUri() + "/trip/api/insertGenerated",
          request2,
          String.class
        );
    ...
    task.whenComplete(
      new BiConsumer<ComplexResponse, Throwable>() {
        @Override
