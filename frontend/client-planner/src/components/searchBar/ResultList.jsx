...
function ResultList(props) {
    ...
    const getResult = () => {
        axios
            .get("http://localhost:8080/search/both/" + props.text, {
    ...
    return (
        <MDBListGroup className={style.resultList}>
            ...
            {props.text && (
                <MDBListGroupItem className={style.item + " px-3 square border"}>
                    <FontAwesomeIcon className={style.icon} icon={faSearch}></FontAwesomeIcon> &nbsp; &nbsp; See more
                    result for "{props.text}"
                </MDBListGroupItem>
            )}
        </MDBListGroup>
    );
