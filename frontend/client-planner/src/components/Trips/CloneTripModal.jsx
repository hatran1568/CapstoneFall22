...
function CloneTripModal(props) {
  const { onSubmit, onHide, tripId, tripStartDate, tripEndDate, ...rest } = props;
  const [startDate, setStartDate] = useState();
  const [endDate, setEndDate] = useState();
  const diff = new Date(tripEndDate).getDate() - new Date(tripStartDate).getDate();
  const close = () => {
    setStartDate();
    onHide();
  };
  const submit = () => {
    ...
    var formattedDate = new Date(startDate - tzoffset).toISOString().substring(0, 10);
    if (tripId) {
      const data = {
        tripId: tripId,
    ...
    <>
      <Modal {...rest} size='md' aria-labelledby='contained-modal-title-vcenter' centered>
        <Modal.Body className={style.cloneModal}>
          <button className={`btn-close ${style.closeBtn}`} onClick={close}></button>
          <div className={style.cloneModalBody}>
            <div className={style.cloneModalTitle}>Biến nó thành chuyến đi của bạn</div>
            <div className='row'>
              <div className='col-6'>
            ...
            <div className={style.cloneBtnGroup}>
              <Button variant='outline-dark' onClick={submit} className={style.submitBtn} disabled={startDate == null}>
                Lưu
              </Button>
            </div>
          </div>
