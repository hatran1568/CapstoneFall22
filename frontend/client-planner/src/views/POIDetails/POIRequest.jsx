...
class POIRequest extends Component {
  ...
  updateClick = async (event) => {
    ...
    if (validated) {
      ...
      await axios({
        method: "post",
        url: "http://localhost:8080/api/request/new",
        data: {
          poiId: id,
  ...
  render() {
    ...
    submitBtn.push(
      <div className={style.btnBoxUpdate}>
        ...
        <MDBBtn className={style.updateBtn} onClick={this.updateClick}>Thêm yêu cầu</MDBBtn>
      </div>)
    const headerText = [];
    headerText.push(<h2 style={{'textAlign':'center'}}>Yêu cầu cập nhật Địa điểm</h2>);
    const imageAddBtn = [];
    imageAddBtn.push(
      <MDBContainer>
