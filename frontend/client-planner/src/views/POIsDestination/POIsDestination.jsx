...
function POIsDestination(){
    ...
    useEffect(() => {
        const listResp = async () => {
          await axios.get('http://localhost:8080/api/pois/' + desId + "/" + 0 + "/" + catId + "/" + rating)
          .then(
    ...
    useEffect(() => {
        const listResp = async () => {
          await axios.get('http://localhost:8080/api/pois/' + desId + "/" + catId + "/" + rating + "/count")
          .then(
    ...
    ratingSet.forEach((rate, index) => {
      if (rate == rating)
        filterRatingBox.push(<a onClick={ratingBtnClick} id={rate}>
          <MDBRow>
            <MDBCol md="1" className={style.filterRatingButton}><MDBRadio inline defaultChecked/></MDBCol>
          <MDBCol>
          <FilterRatings 
            rating={rate}
            starDimension="20px"
        ...
        filterRatingBox.push(<a onClick={ratingBtnClick} id={rate}>
          <MDBRow className={style.ratingRow} >
            <MDBCol md="1" className={style.filterRatingButton}><MDBRadio inline/></MDBCol>
          <MDBCol>
          <FilterRatings 
            rating={rate}
            starDimension="20px"
    ...
    filterRatingBox.push(<b><a onClick={ratingBtnClick} id={0} className={style.removeFilterButton}>Show All</a></b>)
    //Pagination   
    ...
    const handlePageClick = (event) => {
        ...
        window.scrollTo(0, 0);
        axios.get('http://localhost:8080/api/pois/' + desId + "/" + event.selected + "/" + catId + "/" + rating)
