...
class POIList extends Component {
  ...
  componentDidMount() {
    ...
    axios
      .get(
        "http://localhost:8080/location/api/pois/list/admin/count/" +
          this.state.currentCatId +
          "/" +
          this.state.currentNameKey,
        {
  ...
  deletePOI = async (event) => {
    ...
    confirm({
      ...
      onOk: async () => {
        await axios.post(
          `http://localhost:8080/location/api/pois/delete/` + poiId,
          {
            headers: {
  ...
  render() {
    ...
    this.state.pois.forEach((entry, index) => {
      ...
      if (website == null || website == "") website = "Chưa có";
      var phone = entry.phoneNumber;
      if (phone == null || phone == "") phone = "Chưa có";
      if (index % 2 == 0)
        poiTableData.push(
          <tr className={style.tableDataGrey}>
            ...
            <th scope="col">
              <a
                className={style.tableNameData}
                href={"../poi?id=" + entry.activityId}
              >
                {name}
              </a>
            </th>
            ...
            <th scope="col" className={style.tableRateData}>
              {entry.rating}
              <FontAwesomeIcon className={style.star} icon={faStar} />
            </th>
            <th scope="col" className={style.tableCat}>
              {entry.categoryName}
            </th>
            <th scope="col" className={style.tableWeb}>
              <a className={style.tableNameData} target="_blank" href={website}>
                {website}
              </a>
            </th>
            <th scope="col" className={style.tablePhone}>
              {phone}
            </th>
            <th scope="col" className={style.tableActions}>
              <a
                className={style.tableIcons}
                href={"./update?id=" + entry.activityId}
              >
          ...
          <MDBCol md={3}>
            <Dropdown>
              <Dropdown.Toggle variant="info">
                ...
                <span id="filterDropdown"> Danh mục: Tất cả</span>
              </Dropdown.Toggle>

              <Dropdown.Menu>
                <Dropdown.Item
                  onClick={this.filterChanged}
                  onMouseUp={this.filterChanged}
                  id={0}
                  active
                  name="Tất cả"
                >
