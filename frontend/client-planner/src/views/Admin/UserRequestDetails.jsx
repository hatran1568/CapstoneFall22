...
function UserRequestDetails(props) {
  ...
  useEffect(() => {
    const listResp = async () => {
      await axios
        .get("http://localhost:8080/api/request/details/" + data, {
  ...
  useEffect(() => {
    const listResp = async () => {
      await axios
        .get("http://localhost:8080/api/request/images/" + data, {
          headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
          withCredentials: true,
        })
        .then((response) => {
  ...
  const approve = (event) => {
    axios.post(`http://localhost:8080/api/request/accept/` + data, {
      headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
      withCredentials: true,
  ...
  const reject = (event) => {
    axios.post(`http://localhost:8080/api/request/reject/` + data, {
      headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
      withCredentials: true,
    }).then(function (response) {
      //window.location.reload();
      ...
      imageBox.push(
        <MDBCard className={style.imageBox}>
          ...
          <div className={style.imageContent}>
            <a className={style.deleteIcon} href={entry} download target="_blank"><FontAwesomeIcon icon={faDownload}/></a>
          </div>
        </MDBCard>
      )
    });
  ...
  return (
    <span key={data}>
      ...
      <MDBModal key={data} show={basicModal} setShow={setBasicModal} tabIndex="-1">
        <MDBModalDialog className={style.modal}>
          <MDBModalContent className={style.modalContainer}>
            ...
            <MDBModalBody className={style.modalBody}>
              <b><i>Đây là những thông tin người dùng yêu cầu chỉnh sửa, bấm vào nút Chỉnh sửa để đến trang chỉnh sửa địa điểm</i></b><br/><br/>
              {req.name != "*" &&
              ...
              {images.length > 0 &&
              <b><i>Ảnh người dùng muốn thêm, tải về để chỉnh sửa<br/></i></b>
              }
              {imageBox}
            </MDBModalBody>
            <MDBModalFooter>
              <div className={style.modalBtn}>
                <a href={"/poi/update?id=" + data} target="_blank"><MDBBtn color="info">
                  Chỉnh sửa
                </MDBBtn></a>
              </div>
              <div className={style.modalBtn}>
