...
class TripBudget extends Component {
  ...
  //Delete Expense
  onOkDeleteHandler = (data) => {
    this.setState({ expenseData: data });
  };
  onOkTotalHandler = (data) => {
    this.setState({ totalBudget: data });
  };
  onOkGraphHandler = (data) => {
    ...
    if (expenseRate >= 100) {
      ...
      progressBar.push(
        <span className={style.budgetExceed}>
          You have exceeded the budget by {exceedExpense}%
        </span>
      );
      ...
      progressBar.push(
        <span className={style.budgetWarning}>
          You are at {expenseRate}% of the set budget
        </span>
      );
    } else if (expenseRate <= 80) {
      progressBar.push(
          ...
          <MDBCard className={style.budgetContainer}>
            <MDBCardBody>
              <span className={style.expenseText}>
                Expenses: {formatter.format(this.state.totalBudget)}
              </span>
              <span className={style.budgetText}>
                Set Budget: {formatter.format(this.state.trip.budget)}
              </span>
              <br />
              {progressBar}
            ...
            <MDBCol md={4} className={style.expenseFilter}>
              <Dropdown>
                <Dropdown.Toggle variant="info">
                  ...
                  <span id="filterDropdown"> Filter: By Date</span>
                </Dropdown.Toggle>
